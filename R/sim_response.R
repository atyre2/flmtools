...
#' test <- simResponse(test, 0.05*x1 + rowSums(0.1*lag_6_24*envVar_6_24), sd = 5)
#' fit <- mgcv::gam(response ~ x1 + s(lag_6_24, by = envVar_6_24), data = test)
#' summary(fit)
#'
#' # poisson response with sine function of lag and an offset
#' test <- simResponse(test, exp(-2 + log(offset) - 0.05*x1 + rowSums(envVar_6_24*sin(lag_6_24/pi))), rpois)
#' fit <- mgcv::gam(response ~ offset(log(offset))  + x1 + s(lag_6_24, by = envVar_6_24), data = test, family = poisson)
#' summary(fit)
#'
